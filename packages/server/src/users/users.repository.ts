...
export class UsersRepository {
  ...
  async addMoodToUser(userId: string, mood: Mood): Promise<UserDocument | null> {
    return this.userModel.findByIdAndUpdate(userId, { $push: { moods: mood } }, { new: true }).exec()
  }
  ...
  async getMoodsByDateRange(userId: string, startDate: Date, endDate: Date): Promise<Mood[]> {
  ...
  async checkDuplicateMoodInHour(userId: string): Promise<boolean> {
    ...
    return user.moods.some((mood: any) => mood.createdAt && mood.createdAt > oneHourAgo)
  }
