...
export class MoodsService implements IMoodService {
  ...
  async createMood(body: CreateMoodDto): Promise<MoodDocument> {
    ...
    const weatherData = await this.getWeather(body.location.lat, body.location.lng)

    const textSentimentRating = await this.getTextSentimentAnalysis(body.textContent)
