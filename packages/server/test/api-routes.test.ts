...
describe('API Routes', { timeout: 30000 }, () => {
  ...
  describe('POST /moods', () => {
    ...
    it('should submit mood with all required fields', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/api/v1/moods').send(moodData).expect(201)
    ...
    it('should submit mood with picture and analyze sentiment from image', async () => {
      const happyImagePath = path.join(__dirname, 'mocks', 'portraits', 'happy.jpg')
  ...
  describe('GET /moods', () => {
    ...
    it('should get all moods from today', async () => {
      const response = await request(app.getHttpServer()).get(`/api/v1/moods`).expect(200)
      ...
      const userMoods = response.body.moods.filter(
        (m: any) => m.textContent === 'First mood' || m.textContent === 'Second mood',
      )
